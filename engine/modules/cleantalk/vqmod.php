...
final class VQMod {
    ...
    public $logFolder = 'engine/modules/cleantalk/logs/';
    public $vqCachePath = 'cleantalk/vqcache/';
    public $modCache = 'mods.cache';
    public $protectedFilelist = 'vqprotect.txt';
    public $pathReplaces = 'engine/modules/cleantalk/pathReplaces.php';
    public $logging = true;
    public $log;
    ...
    public $version_to_delete = null;
    public $version_to_install = null;
    public $work_status = 0; //0 - none, 1 - install, 2 - delete
    public $test = false; // Testing patched
    public $remove = false;
    public $patched_files = null;
    ...
    public function __construct($path = false, $installed_version = null, $remove=null, $logging = true) {
    ...
    private function _checkPermissions() {
        if ($this->work_status == 2) {
            ...
            // Метка, указывающая установку после удаления
            $tmp_work_status = 1;
        }
        if ($this->work_status == 1 || $tmp_work_status == 1) {
            foreach ($this->_mods as $modObject) {
    ...
    public function remove($version_to_delete) {
        $this->version_to_delete = $version_to_delete;
        $this->work_status = 2;
    ...
    public function run() {
        ...
        if (!empty($errors)) {
            ...
            //TODO: Удаляем сначала потом ставим
            if ($this->work_status == 2) {
                // Удаляем
                foreach ($this->_mods as $modObject) {
                    if ($modObject->version == $this->version_to_delete) {
    ...
    public function addMod($modObject, $path, $mods) {
        $fileHash = sha1_file($path);
        $fileData = file_get_contents($path);

        if ($this->_checkMatch($path, $path)) {
            if ($this->work_status == 1) {
                $modObject->applyMod($mods, $fileData);
            } elseif ($this->work_status == 2) {
                $modObject->removeMod($mods, $fileData);
            }
        }

        if (sha1($fileData) != $fileHash) {
            // Вторая правка - меняю кэш файла на оригинал - $sourcePath
            if ($this->test == false) {
                $writePath = $path;
     ...
     */
    public function modCheck($sourcePath) {
    ...
    private function _getVersion($name) {
        $a = strrpos($name, '/') + 1;
    ...
    private function _setVersionInKey($list) {
        $output = array();

        foreach ($list as $f) {
            $version = $this->_getVersion($f);
            $output[$version] = $f;
        }
    ...
    private function _getMods() {
        ...
        } elseif ($this->installed_version < $this->_getVersion($max_version)) {

            //TODO: удаляем установленную версию
        ...
        //$this->_modFileList = $this->setVerInKey($this->_modFileList);
        //TODO: Сортировать массив _modFileList по версиям начиная с последней
        ...
        /* foreach ($this->_modFileList as $file) {
          ...
          $xml_folder_time = filemtime($this->path('xml'));
...
class VQModObject {
                            ...
                            if (!$mod['search']->indexes() || ($mod['search']->indexes() && in_array($indexCount, $mod['search']->indexes()))) {
                                ...
                                switch ($mod['search']->position) {
                                    case 'before':
                                        ...
                                        $aa = $mod['add']->getContent();
                                        $bb = explode("\n", $aa);
                                        # Additional index to sync with $tmp lines
                                        $last_line_index = 0;
                                        if ($bb[count($bb)] == '')
                                            $last_line_index = 1;

                                        $count_lines = count($bb) - $last_line_index;
                                        $add_lines = null;
                                        for ($i = $offset - $count_lines; $i < $offset; $i++) {
                                            if (trim($bb[$i - $offset + $count_lines]) == trim($tmp[$i]))
                                                $add_lines[] = $i;
                                        }
                                        
                                        if (count($add_lines) != $count_lines) {
                                            $tmp[$offset] = empty($tmp[$offset]) ? $mod['add']->getContent() : $mod['add']->getContent() . $tmp[$offset];
                                        } else {
                                            $this->_vqmod->log->write('Couldn\'t apply patch: ' . $aa, $this);
                                        }
                                    ...
                                    case 'after':
                                        ...
                                        $count_lines = count($bb);
                                        $add_lines = null;
                                        for ($i = $offset + 1; $i < $offset + $count_lines; $i++){
                                            if (trim($bb[$i - $offset]) == trim($tmp[$i]))
                                                $add_lines[] = $i;
                                        }
                                        
                                        if (count($add_lines) + $first_line_index != $count_lines) {
                                            $tmp[$offset] = $tmp[$offset] . $mod['add']->getContent();
                                        } else {
                                            $this->_vqmod->log->write('Couldn\'t apply patch: ' . $aa, $this);
                                        }
    ...
    public function removeMod($mods, &$data) {
        if ($this->_skip)
            return;
        $tmp = $data;
                            ...
                            if (!$mod['search']->indexes() || ($mod['search']->indexes() && in_array($indexCount, $mod['search']->indexes()))) {
                                ...
                                switch ($mod['search']->position) {
                                    case 'before':
                                        ...
                                        if (count($unset_lines) == $count_lines) {
                                            foreach ($unset_lines as $idx)
                                                unset($tmp[$idx]);
                                        } else {
                                            $log_message = 'Couldn\'t remove patch: ' . $aa;
                                            $this->_vqmod->log->write($log_message, $this);
                                        }
                                    ...
                                    case 'after':
                                        ...
                                        # Removes lines only if all lines matched to Data
                                        if (count($unset_lines) + $first_line_index === $count_lines) {
                                            foreach ($unset_lines as $idx)
                                                unset($tmp[$idx]);
                                        } else {
                                            $log_message = 'Couldn\'t remove patch: ' . $aa;
                                    ...
                                    default:
                                        if (!empty($mod['add']->offset)) {
                                            for ($i = 1; $i <= $mod['add']->offset; $i++) {
                                                if (isset($tmp[$lineNum + $i])) {
                                                    $tmp[$lineNum + $i] = '';
                                                }
                                            }
                                        ...
                                        if ($mod['search']->regex == 'true') {
                                            $tmp[$lineNum] = preg_replace($mod['add']->getContent(), $mod['search']->getContent(), $line);
                                        } else {
                                            $tmp[$lineNum] = str_replace($mod['add']->getContent(), $mod['search']->getContent(), $line);
                                        }
                                        break;
                                }
                            }
...
class Error {
    ...
    static public function add($error) {
        self::$errors[] = $error;
    }
